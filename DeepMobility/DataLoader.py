...
class DataLoader(object):
    ...
    def data_load(self):
        ...
        pad = torch.zeros([1,region_att[2].shape[1]])
        if '1000' in self.args.region_feature:
            pad[0][0] = 1000
        elif '100' in self.args.region_feature:
            pad[0][0] = 100

        region_att[2] = torch.cat((region_att[2], pad),dim=0)

        # data load
        init_prob = np.load('data/{}/{}m/init_distribution.npy'.format(self.args.dataset, self.args.resolution)).tolist()
        
        with open('data/{}/{}m/LocIndex2lonlat.json'.format(self.args.dataset,self.args.resolution),'r') as f:
            LocIndex2lonlat = json.load(f)
        LocIndex2lonlat = {int(k):LocIndex2lonlat[k] for k in LocIndex2lonlat}

        with open('data/{}/{}m/RegionIndex2lonlat.json'.format(self.args.dataset,self.args.resolution),'r') as f:
        ...
        loclonlat = torch.tensor([LocIndex2lonlat[k] for k in range(self.args.total_locations)]+[(0.0,0.0)])
        ...
        return data, data_test, region2loc, loc2region, region_att, loc_att, init_prob, loclonlat, LocIndex2lonlat, RegionIndex2lonlat, real_OD
